...
| 123456789101112131415 | namespaceops{classReshape:publicOpDef{public:explicitReshape(constchar*name):OpDef(name){...this->Input("shape").ParamType(REQUIRED)....ValueDepend(REQUIRED)// 声明 ReShape算子的shape输入为数据依赖输入...}};OP_ADD(Reshape);}// namespace ops |
| --- | --- |
