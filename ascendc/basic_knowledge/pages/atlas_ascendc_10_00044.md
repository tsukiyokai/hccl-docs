...
- __host____host__执行空间限定符声明一个函数，它具有如下属性：只能在Host侧执行只能被Host侧函数调用__global__ 和__host__不能一起使用__host__限定符是可选项，无函数执行空间限定符定义的函数，默认是host函数。__aicore__ int f() {}
