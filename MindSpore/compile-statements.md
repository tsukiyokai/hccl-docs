...
示例3： 将报错：UnboundLocalError: The local variable 'z' defined in the 'while' loop body cannot be used outside of the loop body. Please define variable 'z' before 'while'.
...
`def`用于定义函数，后接函数标识符名称和圆括号`()`，括号中可以包含函数的参数。 使用方式：`def function_name(args): statements...`。
...
值得注意的是，with语句中使用的类需要有装饰器@ms.jit_class修饰或者继承于nn.Cell。
