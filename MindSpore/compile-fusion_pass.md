...
算子融合通过将多个独立算子组合成一个更大、更复杂的算子，来减少运行时内存访问、提高计算效率。
...
因此对于融合算子相关优化提供了对应接口，帮助用户自定义融合策略进行调试。

## 调试接口

当前算子融合相关优化Pass已经纳入图算优化控制的范围。提供了环境变量`MS_DEV_GRAPH_KERNEL_FLAGS`可以控制相关图算优化Pass的开关，包括：
...
在`[INFO]`信息中，提供了所有支持自定义开关的Pass列表。用户可以通过`export GLOG_v=1`来生成`[INFO]`信息。在`[INFO]`信息中，用户通过搜索`graph kernel pass`来获取融合Pass列表。

对于单个Pass，也可以通过日志信息确认它是否使能，比如：
